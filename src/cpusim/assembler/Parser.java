 ...
 * 4.) Made a new advanceWithoutReplacingLabels() method that is used when making the first pass 
 * through the included file
 * 5.) Added a hashmap of globals and a corresponding boolean that is true if the labels have already
 * been declared
...
import cpusim.assembler.AssemblyException.ImportError;
import cpusim.assembler.AssemblyException.InvalidTokenException;
import cpusim.assembler.AssemblyException.NameSpaceError;
import cpusim.assembler.AssemblyException.SyntaxError;
import cpusim.assembler.AssemblyException.TypeError;
import cpusim.assembler.AssemblyException.UndefinedOperandError;
...
public class Parser
{
    ...
    private void advance() throws AssemblyException.InvalidTokenException, SyntaxError
    {
        ...
        if (token != null && !token.isLegal()) {
            throw new AssemblyException.InvalidTokenException("Error: non-sensical token was found: \"" +
                    token.contents + "\"",
                    token);
        }
    }
     ...
     */
    private void advanceWithoutReplacingLabels() throws InvalidTokenException, SyntaxError{
        //grab the next token
        token = scanner.getNextToken();
    ...
    private void updateIncludeStacks(){
        ...
        if (!this.includeFileNames.isEmpty()){
            //if the file we are scanning is not on the top of our stack, pop that filename
    ...
    //-------------------------------
    private void Include() throws AssemblyException.SyntaxError, AssemblyException.InvalidTokenException, ImportError, NameSpaceError
    {
        ...
        //make initial pass through the included file
        includeFirstPass(token.filename);
        
        //scan the file again now that the uniqueLabelsHash and filename has been pushed onto their
     ...
     */
    private void includeFirstPass(String filename) throws InvalidTokenException, SyntaxError, NameSpaceError{
        ...
        while(scanner.thisFileIsCurrentlyBeingScanned(filename)){
		    ...
		    if (includeLabelHash.containsKey(token)) {
		        throw new AssemblyException.NameSpaceError("The label \"" +
                    ...
                    Token vt = new Token(token.filename, Token.Type.VAR, token.columnNumber, token.columnNumber,
                            token.offset, token.contents.substring(0, token.contents.length()-1), token.isLegal);
                    
                    //make a label token with a unique name
                    Token nt = new Token(token.filename, token.type, token.columnNumber, token.columnNumber,
                            token.offset, "IL$"+this.uniqueIncludeLabelNumber + machine.getLabelChar(),
                            token.isLegal);
                    
                    //make a variable label token (without the labelChar) with a unique name
                    Token nvt = new Token(token.filename, Token.Type.VAR, token.columnNumber, token.columnNumber,
                            token.offset, "IL$"+this.uniqueIncludeLabelNumber, token.isLegal);
    ...
    private void Global() throws InvalidTokenException, SyntaxError{
        ...
        globals.put(new Token(token.filename, Token.Type.LABEL, token.lineNumber, token.columnNumber,
                token.offset, token.contents + machine.getLabelChar(), token.isLegal), false);
    ...
    private void Comments_and_EOLs() throws AssemblyException.SyntaxError, InvalidTokenException
    {

        do {
            if (! currentTokenHasOneOfTypes(
                             new Token.Type[]{Token.Type.COMMENT, Token.Type.EOL, Token.Type.EOF})) {
            ...
            advance();
        } while (currentTokenHasOneOfTypes(new Token.Type[]{Token.Type.COMMENT, Token.Type.EOL}));
    ...
    private void Equ_and_Include_and_macro_and_global_part() throws AssemblyException
    {
        ...
        while (currentTokenHasOneOfTypes(new Token.Type[]{Token.Type.VAR, Token.Type.MACRO,
                                                        Token.Type.INCLUDE, Token.Type.GLOBAL})) {
            if (token.type == Token.Type.INCLUDE) {
                Include();
                //hack--if the included file doesn't start with an EOL or comment
    ...
    private void Equ_decl() throws AssemblyException.SyntaxError, NameSpaceError, InvalidTokenException, UndefinedOperandError
    {
        //Symbol
        ...
        //Operand
        if (! currentTokenHasOneOfTypes(new Token.Type[]{Token.Type.VAR, Token.Type.CONSTANT}))
    ...
    private void Macro_decl() throws AssemblyException.SyntaxError, AssemblyException.NameSpaceError, InvalidTokenException, ImportError
    {
        //Macro ...
        ...
        if (! currentTokenHasOneOfTypes(new Token.Type[]{Token.Type.VAR, Token.Type.OPCODE})) {
    ...
    private void Instr_part() throws AssemblyException.InvalidTokenException, SyntaxError, ImportError, NameSpaceError, UndefinedOperandError, TypeError, AssemblyException.InvalidOperandError
    {
    ...
    //-------------------------------
    //...Symbol is either an OPCODE, DATA or a VAR (representing a macro call)
    //Instr ->
    //  (Label Comments-and-EOLs)* [Label] Symbol [Operand ([","] Operand)*] [Comment]
    private void Instr() throws AssemblyException.NameSpaceError, AssemblyException.SyntaxError, InvalidTokenException, UndefinedOperandError, TypeError, AssemblyException.InvalidOperandError
    {
    ...
    private void parseAsciiPseudoinstruction(InstructionCall node) throws AssemblyException.SyntaxError, InvalidTokenException, AssemblyException.InvalidOperandError
    {
    ...
    private void parseRegularInstruction(InstructionCall node) throws AssemblyException.SyntaxError, InvalidTokenException, TypeError
    {
        node.sourceLine = new SourceLine(token.lineNumber,token.filename);
...
//            }
        ...
        check(currentTokenHasOneOfTypes(new Token.Type[]{Token.Type.COMMENT, Token.Type.EOL, Token.Type.EOF}),
    ...
    private void check(boolean condition, Token token, String message) throws AssemblyException.TypeError
    {
        if( ! condition )
            throw new AssemblyException.TypeError(message, token);
    }

    private void parseDataPseudoinstruction(InstructionCall node) throws AssemblyException.SyntaxError, InvalidTokenException, TypeError
    {
        node.sourceLine = new SourceLine(token.lineNumber,token.filename);
        if (node.comment.equals(""))
            node.comment = "\t"; //indent lines with no labels
        ...
        //first operand
        check(currentTokenHasOneOfTypes(new Token.Type[]{Token.Type.VAR, Token.Type.CONSTANT}),
        ...
        node.comment += " " + token.contents;
        advance();
        if (token.contents.equals(",")) advance(); //ignore commas
        ...
        if( token.contents.equals("[") ) { //there are more operands
            ...
            while (token.type == Token.Type.VAR || token.type == Token.Type.CONSTANT) {
                ...
                node.comment += " " + token.contents;
                advance();
                if (token.contents.equals(",")) advance();
            }
    ...
    //-------------------------------
    private void macroCall() throws AssemblyException.InvalidTokenException, SyntaxError, UndefinedOperandError, NameSpaceError
    {
	...
	private void firstPass(List<Token> specializedMacroBody,
                                                List<Token> localLabels, int j) throws AssemblyException.NameSpaceError {
		Token t = specializedMacroBody.get(j);
	...
	//-------------------------------
	private MacroDef getMdef(Token macroDefKey, List<Token> macroArgs) throws AssemblyException.UndefinedOperandError
    {
		MacroDef mdef = macros.get(macroDefKey);
        assert mdef != null : "null macro def in getMdef";

        if (mdef.parameters.size() != macroArgs.size()) {
            throw new AssemblyException.UndefinedOperandError("Wrong number of arguments " +
    ...
    //and when there is a parameter, replace it with an argument.
    //The cloning is done so that the original MacroDef body can be left alone
	private List<Token> getMacroArgs() throws AssemblyException.SyntaxError, InvalidTokenException {
    ...
    //	one EQU 1
    //	one EQU 8
    private boolean checkEquNotAlreadyUsed(Token token) throws AssemblyException.NameSpaceError
    {
    ...
    //	two EQU 2
    //	one EQU two
    private void checkEquDefined(Token token) throws AssemblyException.UndefinedOperandError
    {
        ...
        throw new AssemblyException.UndefinedOperandError("EQU value \"" + token.contents +
