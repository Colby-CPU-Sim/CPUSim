...
public class HelpController implements Initializable {
	...
	public static final String[][] nameURLPairs = {
		...
		{".ascii Statements", pref+"/specifications/assemblyLanguage/asciiInstrs.html"},
                {".global Statements", pref+"/specifications/assemblyLanguage/globalInstrs.html"},
					...
					public void changed(ObservableValue<? extends State> arg0,
						...
						if (newState == State.SUCCEEDED) {
							WebEngine webEngine = webView.getEngine();
                                                        ...
                                                        searchController.setDocumentCopy(webEngine.getDocument());
                                                        //loads tags on change. Somewhat of a hack for handeling switching 
                                                        //between files when searching is active. Highlights words 
                                                        //imediately when displaying a new file in the web view
                                                        searchController.addHighlightTagsToBody(webEngine.getDocument());
	 ...
	 */
        private void initializeSearchGuiElements() {
            ...
            helpPane.setOnKeyTyped(new EventHandler<KeyEvent>() {
                @Override
	 ...
	 */
	public void initializeSearchField() {
            searchTF.getStylesheets().add(getClass().getResource("/cpusim/gui/help/SearchComboBox.css").toExternalForm());
		searchTF.setOnKeyReleased(new EventHandler<KeyEvent>() {
			@Override
			public void handle(KeyEvent event) {
                    WebEngine webEngine = webView.getEngine();
                    searchController.initializeSearch(webEngine.getDocument(), searchTF.getText());
                    Map<String,Integer> occurrenceMap = searchController.getSortedNameOccurrenceMap();
                    initializeSearchList(occurrenceMap);
                    if(event.getCode().equals(KeyCode.ENTER)){
                        System.out.println("HERE");
                        searchList.focusModelProperty().get().focus(0);
                        String fileNameNoOccurrences = searchList.getFocusModel().focusedItemProperty().getValue().split(" ",2)[1];
                        //webEngine = webView.getEngine();
                        webEngine.load(getClass().getResource(urls.get(fileNameNoOccurrences)).toExternalForm());
	...
	/**
	 * initializes searchList a ListView object with the filenames containing the searched string, 
	 * filenames appear in a descending order corresponding to the number of
	 ...
	 * @param nameOccurrenceMap		contains the file name and its location
	 * 
	 */
        private void initializeSearchList(Map<String,Integer> nameOccurrenceMap) {
	 ...
	 */
        public void initializeSearchChoiceBox(){
            searchModeCB.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
                @Override
            ...
            searchBox.getStylesheets().add(getClass().getResource("/cpusim/gui/help/SearchComboBox.css").toExternalForm());
        }
