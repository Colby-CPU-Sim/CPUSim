 ...
 * 
 * Changes:	onReplaceAllButtonClicked() was rewritten so that undoing a replace-all would undo 
...
public class FindReplaceController implements Initializable {
		...
		if (updateInstances()) {
			...
			for (int i = instancesWOOverlaps.size()-1; i >= 0 ; i--) {
				...
				replaceItoJWith(I, J, replaceWithTF.getText()); 
                        ...
                        CodeArea ta = (CodeArea) desktop.getTextTabPane().getSelectionModel(
	...
	public StyledTextArea validAreaToSearch() {
		...
		CodeArea selectedTextArea = (CodeArea) currTab.getContent();
