...
public abstract class MoreBindings {
    ...
    public static <E, F> ObservableList<E> orderedMap(ObservableList<? extends F> list, Function<? super F, ? extends E> mapper) {
        return new OrderBackedMappedList<E, F>(list, mapper);
    }

    static class OrderBackedMappedList<E, F> extends TransformationList<E, F> {

        private final Function<? super F, ? extends E> mapper;

        private final Set<IndexWrapper> backing;

        public OrderBackedMappedList(ObservableList<? extends F> source, Function<? super F, ? extends E> mapper) {
            super(source);
            this.mapper = mapper;
            backing = new TreeSet<>();
        }
        ...
        public E get(int index) {
            ...
            Iterator<Map.Entry<F, E>> iiter = backing.entries().iterator();

            for (int i = 0; i < index && iiter.hasNext(); ++i, iiter.next())
                ;
        ...
        protected void sourceChanged(ListChangeListener.Change<? extends F> c) {
            fireChange(new ListChangeListener.Change<E>(this) {
                ...
                public List<E> getRemoved() {
                    ...
                    Iterator<Map.Entry<F, E>> iiter = backing.entries().iterator();
                    Iterator<? extends F> riter = c.getRemoved().iterator();

                    while (riter.hasNext() && iiter.hasNext()) {
                        F r = riter.next();

                        while (iiter.hasNext()) {
                            Map.Entry<F, E> entry = iiter.next();
        ...
        // FIXME use this wrapper in a TreeSet ordered by the index

        private class IndexWrapper implements Comparable<IndexWrapper>
        {
            private int index;
            private final WeakReference<F> key;

            private final E value;

            IndexWrapper(int index, F key, E value) {
                this.index = index;
            ...
            public boolean equals(Object o) {
                ...
                IndexWrapper that = (IndexWrapper) o;
            ...
            @Override
            public int compareTo(@Nonnull IndexWrapper o) {
                checkNotNull(o);
